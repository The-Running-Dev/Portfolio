...
export class PreBuild {
  ...
  private createDataIndex(): void {
    ...
    try {
      ...
      if (jsonFiles.length === 0) {
        ...
        console.log(`[INFO] Created data/index.ts with ${jsonFiles.length} JSON export(s)`);
      }
    } catch (error) {
      console.error(`[ERROR] Failed to Create Data Index: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  private setupConfig(): void {
    try {
      ...
      if (!fs.existsSync(yamlConfigPath)) {
        ...
        fs.writeFileSync(jsonConfigPath, JSON.stringify(defaultConfig, null, 2), 'utf-8');
      ...
      console.error(`[ERROR] Failed to Convert globalConfig.yml: ${error instanceof Error ? error.message : String(error)}`);

      // Instead of throwing, create a minimal config and continue
      console.warn('[WARN] Creating minimal configuration to continue build...');
      ...
      try {
        ...
        fs.writeFileSync(jsonConfigPath, JSON.stringify(minimalConfig, null, 2), 'utf-8');

        console.log(`[INFO] Created Minimal globalConfig.json for Fallback`);
      } catch (fallbackError) {
        console.error(`[ERROR] Could Not Create Fallback Config: ${fallbackError instanceof Error ? fallbackError.message : String(fallbackError)}`);
  ...
  private copyMarkdown(): void {
    ...
    if (!this.config.preBuild?.projectRoot) {
      console.warn('[WARN] Project Root Path Not Configured, Skipping Markdown Copy');
    ...
    if (!fs.existsSync(this.config.preBuild.projectRoot)) {
      console.warn(`[WARN] Project Root Directory Not Found: ${this.config.preBuild.projectRoot}`);
  ...
  private processYamlToJson(): void {
    ...
    yamlFiles.forEach((file) => {
      try {
        ...
        console.error(`[ERROR] Failed to Process ${file}: ${error instanceof Error ? error.message : String(error)}`);
      }
    });

    console.log(`[INFO] YAML to JSON Conversion Completed: ${processedCount} File(s) Processed`);
  ...
  private loadConfig(): GlobalConfig {
    try {
      ...
      if (!fs.existsSync(globalConfigPath)) {
        console.warn('[WARN] globalConfig.json Not Found, Using Default Configuration');
      ...
      if (!configData) {
        console.warn('[WARN] globalConfig Data is Null or Undefined, Using Defaults');
        return this.getDefaultConfig();
      }
      ...
      console.error(`[ERROR] Failed to Load Config: ${error instanceof Error ? error.message : String(error)}`);
  ...
  public process(): void {
    try {
      ...
      console.error(`[ERROR] Pre Build Process Failed: ${error instanceof Error ? error.message : String(error)}`);
