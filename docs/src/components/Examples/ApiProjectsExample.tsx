...
export const ApiProjectsExample: React.FC = () => {
  ...
  if (!enabled) {
    return (
      <div style={{ padding: '1rem', backgroundColor: '#f5f5f5', borderRadius: '8px', margin: '1rem 0' }}>
        <h3>API Data Fetching</h3>
        <p><strong>Status:</strong> Disabled (by default)</p>
        <p>
          The API hook is disabled by default for security and performance reasons. 
          To enable it, you need to:
        </p>
        <ol>
          <li>Set the feature flag <code>apiDataFetching: true</code> in GlobalConfig.json</li>
          <li>Pass <code>enabled: true</code> to the useApi hook</li>
        </ol>
        <p><em>This demonstrates the disabled state behavior.</em></p>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: '1rem', border: '1px solid #ddd', borderRadius: '8px', margin: '1rem 0' }}>
      ...
      {data && !loading && (
        <div>
          <p style={{ color: '#008800' }}>✅ Successfully loaded {Array.isArray(data) ? data.length : 'project'} project(s)</p>
          <details>
