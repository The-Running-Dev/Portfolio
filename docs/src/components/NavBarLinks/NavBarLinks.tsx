...
const NavBarLinksContent: React.FC<{ config: NavBarLinksConfig; enabled: boolean }> = ({ config: activeConfig, enabled }) => {
  ...
  if (!enabled || !activeConfig.links || activeConfig.links.length === 0) {
    return null;
  }
    ...
    return (
      ...
      >
        ...
        <div className="navbar-links__dropdown-menu">
          {activeConfig.links.map((link) => renderLink(link, true, link.href))}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={containerClass}>
      {activeConfig.links.map((link) => renderLink(link, false, link.href))}
    </div>
  );
};
