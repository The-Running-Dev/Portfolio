...
const TextSizeSwitcher: React.FC = () => {
  ...
  if (!featuresConfig.enableTextSizeSwitcher) {
    return null;
  }
  ...
  const applyTextSize = (sizeName: string) => {
    try {
      ...
      document.documentElement.style.setProperty('--text-size-scale', textSize.scale.toString());
      ...
      setTimeout(() => {
        if (!document.documentElement.classList.contains(textSize.className)) {
          document.documentElement.classList.add(textSize.className);
        }
      }, 50);
