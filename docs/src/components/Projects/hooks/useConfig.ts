...
export function useConfig(): ProjectsConfig {
  return useMemo(() => {
    try {
      ...
      const provider = globalConfig.projects?.provider || DEFAULT_PROJECTS_CONFIG.provider;
      const location = globalConfig.projects?.location || DEFAULT_PROJECTS_CONFIG.location;
