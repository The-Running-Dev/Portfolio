...
export default function useConfig({
  ...
  const filteredData = useMemo(() => {
      ...
      filteredCategories = processedCategories
        .map(category => ({
          ...
          subCategories: category.subCategories
            .map(subCategory => ({
              ...subCategory,
              projects: subCategory.projects.filter(project =>
    ...
    if (selectedCategory && selectedCategory !== 'all' && selectedCategory !== 'most-recent') {
      if (selectedCategory === 'all-tags') {
        ...
        filteredCategories = processedCategories
          .filter(cat => cat.category.toLowerCase() === categoryName.toLowerCase())
          .map(cat => ({
            ...cat,
            subCategories: cat.subCategories.filter(sub => sub.name.toLowerCase() === subCategoryName.toLowerCase()),
          }));
      } else {
...
function processData(rawData: any[]) {
  ...
  // Process categories
  const processedCategories: ProcessedCategory[] = categories.map(category => ({
    ...
    .sort((a, b) => {
      const totalProjectsA = a.subCategories.reduce((sum, sub) => sum + sub.projects.length, 0);
      const totalProjectsB = b.subCategories.reduce((sum, sub) => sum + sub.projects.length, 0);
...
function calculateStats(categories: ProcessedCategory[]): ProjectStats {
  ...
  const totalTechnologies = categories.reduce((count, cat) => count + cat.subCategories.length, 0);
    ...
    category.subCategories.forEach(subCategory => {
      ...
      subCategory.projects.forEach(project => {
        if (project.lastModified) {
          ...
          const age = (new Date().getTime() - projectDate.getTime()) / (365 * 24 * 60 * 60 * 1000);
  ...
  const averageAge = projectsWithDates > 0 ? Math.round(totalAge / projectsWithDates) : 0;
...
function generateCategoryOptions(categories: ProcessedCategory[]): FilterOption[] {
  // Calculate total projects for "All" option
  const totalProjects = categories.reduce(
    (total, cat) => total + cat.subCategories.reduce(
      (subTotal, sub) => subTotal + sub.projects.length, 0
  ...
  return [
    ...
    ...categories.map(cat => {
      const categoryProjectCount = cat.subCategories.reduce(
        (count, sub) => count + sub.projects.length, 0
      );
      return {
        key: cat.category,
...
function generateDateOptions(categories: ProcessedCategory[]): FilterOption[] {
  ...
  const mostRecentCount = allDates.filter(date => date >= sixMonthsAgo).length;
  const lastMonthCount = allDates.filter(date => date >= oneMonthAgo).length;
  const last3MonthsCount = allDates.filter(date => date >= threeMonthsAgo).length;
  const last6MonthsCount = allDates.filter(date => date >= sixMonthsAgo).length;
  ...
  if (allDates.some(date => date >= oneMonthAgo)) {
    options.push({ key: "last-month", label: `Last Month (${lastMonthCount})` });
  }

  if (allDates.some(date => date >= threeMonthsAgo)) {
    options.push({ key: "last-3-months", label: `Last 3 Months (${last3MonthsCount})` });
  }

  if (allDates.some(date => date >= sixMonthsAgo)) {
    options.push({ key: "last-6-months", label: `Last 6 Months (${last6MonthsCount})` });
  }
...
function applyDateFiltering(categories: ProcessedCategory[], dateRange?: string): ProcessedCategory[] {
...
function generateTagOptions(categories: ProcessedCategory[]): FilterOption[] {
  ...
  const totalProjectsWithTags = categories.reduce(
    (total, cat) => total + cat.subCategories.reduce(
      (subTotal, sub) => subTotal + sub.projects.filter(project => project.tags && project.tags.length > 0).length, 0
    ), 0
  );

  return [
    { key: "all-tags", label: `All (${totalProjectsWithTags})`, category: "tag", count: totalProjectsWithTags },
    ...tagOptions,
  ];
}
