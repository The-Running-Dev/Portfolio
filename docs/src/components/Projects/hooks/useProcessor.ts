...
export function useProcessor(
  ...
  const processedData = useMemo(() => {
    ...
    try {
      ...
      const processingError = err instanceof Error ? err : new Error('Unknown processing error');
...
function getEmptyProcessedData(): ProcessedProjectData {
  return {
    ...
    tagOptions: [{ key: 'all-tags', label: 'All (0)', category: 'tag', count: 0 }],
    stats: {
      totalProjects: 0,
      recentProjects: 0,
      totalTechnologies: 0,
      ...
      return {
        ...
        subCategories: (Array.isArray(category.subCategories) ? category.subCategories : [])
          ...
          .map((sub: any, subIndex: number) => ({
            name: sub.name || `Unknown-Subcategory-${categoryIndex}-${subIndex}`,
